...
var initialbarPosLeft = bar[0].offsetLeft;
...
window.alert("Press Enter to PLay  |   Use A & D to move the bars");

// bar movement


for (let i = 0; i < 2; i++) {
    let cur = bar[i].offsetLeft;
    bar[i].style.left = cur + "px";
}
...
window.addEventListener("keydown", function(e) {
    ...
    for (let j = 0; j < 2; j++) {
        let cur = parseFloat(bar[j].style.left);
        let new_pos;
        if (e.keyCode == 65) {
            if (cur > 25) {
                new_pos = cur - 25;
            ...
            if (cur < (box.offsetWidth - bar[j].offsetWidth - 20)) {
                new_pos = cur + 25;
            }
        }

        bar[j].style.left = new_pos + "px";
    }
    ...
    if (event.keyCode == 13) {
        ...
        var interval = setInterval(function() {
            if (goUp == true) {
                ...
                if (parseFloat(ball.style.top) <= bar[0].offsetHeight) {
                    if (parseFloat(ball.style.left) > parseFloat(bar[0].style.left) && parseFloat(ball.style.left) < parseFloat(bar[0].style.left) + bar[0].offsetWidth) {
                        player2Score++;
                        goUp = false;
                        goDown = true;
                    ...
                    if (parseFloat(ball.style.top) <= 5) {
                        ...
                        setTimeout(function() {
                            ...
                            bar[0].style.left = initialbarPosLeft + "px";
                            bar[1].style.left = initialbarPosLeft + "px";
                        }, 500)
            ...
            if (goDown == true) {
                ...
                if (parseFloat(ball.style.top) >= (box.offsetHeight - bar[0].offsetHeight - ball.offsetHeight)) {
                    if (parseFloat(ball.style.left) > parseFloat(bar[0].style.left) && parseFloat(ball.style.left) < parseFloat(bar[0].style.left) + bar[0].offsetWidth) {
                        player1Score++;
                        goDown = false;
                        goUp = true;
